...
class Nodes:
    ...
    def supervisor(self, state: GraphState):
        logger.info("***IN SUPERVISOR NODE***")
        ...
        if current_round == 1 and "initial" not in votes:
            logger.info("Triggering initial vote before debate starts.")
        ...
        if current_round == DEBATE_ROUND_COUNT // 2 and "mid" not in votes:
            logger.info(f"Triggering mid-debate vote at round {current_round}.")
        ...
        if current_round > DEBATE_ROUND_COUNT and "final" not in votes:
            logger.info("Debate rounds complete. Transitioning to final vote.")
        ...
        logger.info(f"Round {current_round} — speaker: {speaker}")
    ...
    def agent_speak(self, state: GraphState):
        logger.info("***IN AGENT SPEAK NODE***")
    ...
    def update_short_memory(self, state: GraphState):
        logger.info("***IN UPDATE SHORT MEMORY NODE***")
    ...
    def update_long_memory(self, state: GraphState):
        logger.info("***IN UPDATE LONG MEMORY NODE***")
        
        current_round = state["round"]
        
        # check if it's time to update long memory based on interval or voting phase
        if state["phase"] != "vote" and current_round % LONG_MEMORY_UPDATE_INTERVAL != 0:
            logger.debug(f"Skipping long memory update: round {current_round} not at interval {LONG_MEMORY_UPDATE_INTERVAL}")
            return {"agents": state["agents"], "last_long_mem_update_round": state.get("last_long_mem_update_round") or 0}
        
        logger.info(f"Long memory update triggered at round {current_round}")
        ...
        for agent_name, agent in state["agents"].items():
            ...
            try:
                ...
                logger.info(f"Agent '{agent_name}': summary stored (total: {len(agent['long_mem'])})")
    ...
    def vote(self, state: GraphState) -> Dict[str, Any]:
        ...
        logger.info(f"Vote phase '{vote_label}' complete. Results: { {a: v['vote'] for a, v in round_votes.items()} }")
